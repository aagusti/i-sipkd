...
def form_validator(form, value):
    ...
    if 'id' in form.request.matchdict:
        ...
        q = DBSession.query(RekeningModel).filter_by(id=uid)
        r = q.first()
    else:
        r = None
    q = DBSession.query(RekeningModel).filter_by(kode=value['kode'])
    found = q.first()
    if r:
    ...
    if 'uraian' in value: # optional
        found = RekeningModel.get_by_uraian(value['uraian'])
        if r:
            if found and found.id != r.id:
                err_name()
        elif found:
...
def query_id(request):
    return DBSession.query(RekeningModel).filter_by(id=request.matchdict['id'])
