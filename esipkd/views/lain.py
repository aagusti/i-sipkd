...
def get_units():
    q = DBSession.query(UnitModel.id,UnitModel.uraian).filter(
          UnitModel.kode.like("1.20.05.%")
        ).order_by(UnitModel.uraian)
    return q.all()
...
def view_act(request):
    ...
    if url_dict['act']=='grid':
        ...
        query = DBSession.query(SptpdModel)
        rowTable = DataTables(req, SptpdModel, query, columns)
